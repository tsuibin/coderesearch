...
$versionhost = REQ_PROTOCOL . '://vbc-version.vbulletin.com';
...
if ($_REQUEST['do'] == 'head')
{
	...
	<tr align="center" valign="top">
		<td style="text-align:<?php echo $stylevar['left']; ?>"><a href="http://www.vbulletin-china.cn/" target="_blank"><b><?php echo $vbphrase['admin_control_panel']; ?></b> (vBulletin <?php echo ADMIN_VERSION_VBULLETIN . print_form_middle('[#]license[#]'); ?>)<?php echo iif(is_demo_mode(), ' <b>DEMO MODE</b>'); ?></a></td>
		<td><a href="http://members.vbulletin-china.cn/" id="head_version_link" target="_blank">&nbsp;</a></td>
	...
	<script type="text/javascript" src="<?php echo $versionhost; ?>/version.js?v=<?php echo SIMPLE_VERSION; ?>&amp;id=[#]license[#]"></script>
...
if ($_REQUEST['do'] == 'nav')
{
	...
	echo "\n<div>";
	?><img src="../cpstyles/<?php echo $vbulletin->options['cpstylefolder']; ?>/cp_logo.gif" title="<?php echo $vbphrase['admin_control_panel']; ?>" alt="" border="0" hspace="4" <?php $df = print_form_middle("[#]license[#]"); ?> vspace="4" /><?php
...
print_label_row($vbphrase['useful_links'], '
	...
	<select onchange="if (this.options[this.selectedIndex].value != \'\') { window.open(this.options[this.selectedIndex].value); } return false;" tabindex="1" class="bginput">
		<option value="">-- ' . $vbphrase['useful_links'] . ' --</option>' . construct_select_options(array(
			'vBulletin' => array(
				'http://www.vbulletin-china.cn/' => $vbphrase['home_page'] . ' (vBulletin-China.cn)',
				'http://members.vbulletin-china.cn/' => $vbphrase['members_area'],
				'http://forum.vbulletin-china.cn/' => $vbphrase['community_forums'],
				'http://www.vbulletin-china.cn/docs/html/' => $vbphrase['reference_manual']
			),
			'PHP' => array(
...
if (typeof(vb_version) != "undefined")
{
	...
	{
		document.writeln('<a href="http://forum.vbulletin-china.cn/forum/showthread.ph' + 'p?p=' + vb_announcementid + '" target="_blank">' + construct_phrase(latest_string, vb_version) + '</a><br />' + construct_phrase(current_string, this_vb_version.bold()));
	}
	else
	{
