...
class vB_Group_Collection_Discussion extends vB_Group_Collection_Message
{
	...
	function collection_sql()
	{
		...
		{
			$sql .= " ,(MATCH(gm.titleindex, gm.pagetextindex) AGAINST ('" . $this->registry->db->escape_string($this->filter_searchtext) . "' IN BOOLEAN MODE) ) AS relevance";
		}
		...
		if ($this->filter_searchtext)
		{
			...
			{
				$sql .= " AND (MATCH(gm.titleindex, gm.pagetextindex) AGAINST ('" . $this->registry->db->escape_string($this->filter_searchtext) . "' IN BOOLEAN MODE) )";
			}
			else
			{
				$sql .= " AND ((" . str_replace('--FIELD--', 'gm.title', $this->filter_searchtext). ")
				OR (" . str_replace('--FIELD--', 'gm.pagetext', $this->filter_searchtext). "))";
			}
		}
