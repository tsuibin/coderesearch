...
/*======================================================================*\
|| #################################################################### ||
|| # vBulletin Project Tools [#]version[#] - Licence Number [#]license[#]
|| # ---------------------------------------------------------------- # ||
...
// vB China Start
	function prepare_search_text($query_text, &$errors)
	{
		...
		$ft_search = $vbulletin->options['fulltextsearch'];
		...
		if (!$errors)
		{
			...
			foreach ($matches AS $match)
			{
				if ($match[1][0] == '-')
				{
					...
					{
						// Remove -
						$match[1] = $this->registry->db->escape_string(substr($match[1], 1));
						$new_query_text = "($new_query_text) AND --FIELD-- NOT LIKE '%$match[1]%'";
					}
					continue;
				}
				...
				switch (strtoupper($match[1]))
				{
					...
					default:
						if (!$ft_search)
						{
							$match[1] = $this->registry->db->escape_string($match[1]);
						}
						else
						{
							$match[1] = '(+' . implode(' +', sp_str($match[1])) . ')';
						}
						...
						if ($new_query_text !== '')
						{
							switch ($token_joiner)
							{
								case 'OR':
									...
									{
										$new_query_text .= " OR --FIELD-- LIKE '%$match[1]%'";
									}
									break;
								...
								case 'NOT':
									...
									{
										$new_query_text .= " AND --FIELD-- NOT LIKE '%$match[1]%'";
									}
									break;
								...
								default:
									...
									{
										$new_query_text .= " AND --FIELD-- LIKE '%$match[1]%'";
									}
									break;
							}
						}
						else
						{
							...
							{
								$new_query_text = "--FIELD-- LIKE '%$match[1]%'";
							}
						}
...
// vB China Start
		...
		$this->where['text'] = trim("
				(MATCH(socialgroup.nameindex, socialgroup.descriptionindex) AGAINST ('$value' IN BOOLEAN MODE) )
		");
		}
		else
		{
			$this->where['text'] = trim("
				(" . str_replace('--FIELD--', 'socialgroup.name', $value). ")
				OR (" . str_replace('--FIELD--', 'socialgroup.description', $value). ")
			");
		}
// vB China End
